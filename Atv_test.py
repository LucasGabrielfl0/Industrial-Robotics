...
# Goes to a point and Picks [Mode = true] or Drops [Mode = False] a block
def Jump_to2(px, py, pz, r_dg, mode):
  if(mode ==1):       # Catch Block
    mode = True
    pass
  ...
  m_lite.set_ptpcmd(ptp_mode=2, x= px, y= py, z= pz, r= r_dg)   # Go Above Position
  m_lite.wait(delay=0.5)                                                      # Wait

  m_lite.set_ptpcmd(ptp_mode=2, x= px, y= py, z= pz - DIST_JUMP,  r= r_dg)   # Dive
  m_lite.set_endeffector_suctioncup(enable=True, on=mode)                             # Grab
  m_lite.wait(delay=0.5)                                                                                        # Wait
